...
def process_county():
    ...
    Runs: python detect_surface_anomalies.py --county ATHENS
    """
    try:
        ...
        print(f"Starting surface-anomaly detection for {county}…", flush=True)
        ...
        if result.returncode != 0:
            return jsonify({
                "error": f"detect_surface_anomalies failed for {county}",
...
def health():
    return jsonify({"status": "ok", "worker": "surface_anomalies"}), 200
